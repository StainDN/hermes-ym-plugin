...
- Кнопки быстрых команд под ответом бота (`suggest_buttons`): `/help`, `/new`, `/stop`, `/status`, `/resume`, `/sessions`, `/model`, `/commands` — набор берётся из реестра Hermes (`quick_commands`)
  ...
  platforms:
    ym:
      ...
      extra:
        ...
        quick_commands: true           # false — отключить кнопки быстрых команд
        quick_commands_persist: true   # держать кнопки видимыми после новых сообщений
```
...
- **Треды:** обновление сообщения из треда несёт `thread_id` **внутри объекта `chat`** (integer — timestamp корневого сообщения; в официальной таблице типа `Update` поле не описано, поэтому top-level оставлен как fallback). Адаптер проставляет его в `event.source.thread_id`, и база сама прокидывает его в `metadata`; `send()` и `send_typing()` передают `thread_id` в соответствующие методы Bot API (как int). **Bot API отклоняет `sendText`, в котором одновременно есть `thread_id` и `reply_message_id`**, поэтому при наличии треда цитата-ответ убирается автоматически. Каждый тред — независимая сессия Hermes (`thread_sessions_per_user`, включено по умолчанию; отключить — `extra.thread_sessions_per_user: false`).
- **Авто-новый тред (`thread_replies`):** по умолчанию `group` — сообщение из основного чата группового чата заставляет бота открыть **новый** тред, взяв в качестве якоря `thread_id = message_id` этого сообщения (первое сообщение треда — ответ бота, без дублирующей цитаты). Диалог продолжается внутри треда: последующие сообщения из треда несут тот же `thread_id` и ответы идут в него же. `all` — то же и в личных чатах; `off` — ответ inline, как раньше (входящий `thread_id` не прокидывается). Каналы (`channel`) всегда отвечают inline — треды в них не используются.
- **Кнопки быстрых команд (`quick_commands`):** под каждым **текстовым** ответом бота добавляется клавиатура `suggest_buttons` с командами Hermes. Имена резолвятся из живого реестра `hermes_cli.commands` (fallback — список `QUICK_COMMANDS` в `adapter.py`); нажатие кнопки = пользователь отправляет `/<команда>` (`send_message`-директива), дальше работает штатный обработчик команд Hermes. `persist: true` держит кнопки видимыми после новых сообщений. В **каналах** кнопки не показываются. У Яндекс Bot API **нет сворачиваемого меню-дропдауна** (как у Telegram): все кнопки отображаются сразу, поэтому набор ограничен восемью; полный список команд доступен кнопкой `/commands`. Кнопки не добавляются к отправке файлов/картинок (только `sendText`).
