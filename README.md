...
- Авторизация новых пользователей по pairing-коду: новый пользователь получает код, он же уходит администратору, который одобряет командой `/auth <код>` (`auth_enabled`, `auth_admin_login`)
...
- Доставка из отдельного процесса: `hermes send --to ym:...` и cron-задачи вне гейтвея (`standalone_sender_fn`)
  ...
  platforms:
    ym:
      ...
      extra:
        ...
        auth_enabled: true             # закрыть доступ: только админ + авторизованные
        auth_admin_login: "stain-dn@rudn.ru"   # ваш логин Я.Мессенджера (кому шлются коды)
```
...
- **Фильтр роботов:** сообщения от **своего** бота игнорируются всегда (защита от эхо-цикла). Сообщения от **других** ботов тоже игнорируются, кроме случая `extra.allow_robot_senders: true`, и только в группах/каналах — так читается лента, которую пишет другой бот (например, канал ошибок обмена). Личка от роботов при этом остаётся под действием `dm_policy`, то есть флаг не открывает приватный вход. При включённой `auth_enabled` сообщения роботов из групп не проходят через pairing-гейт (у бота нет кода) — их пускает групповой allowlist.
- **Групповой allowlist принимает id чата:** `group_allow_from` сверяется и с id отправителя, и с id самого чата (`0/0/<guid>`). Второй вариант нужен для лент, которые пишет другой бот: у канала нет «своего» отправителя, которого можно было бы внести в список.
- **Доставка вне гейтвея (`hermes send`, cron в отдельном процессе):** плагин регистрирует `standalone_sender_fn`, поэтому `hermes send --to ym:<логин|chat_id>` шлёт сообщение прямо в Bot API, без живого адаптера. Формы целей: `ym:<логин>` (личка), `ym:0/0/<guid>` (группа/канал), `ym:0/0/<guid>:<thread_id>` (тред — ядро не отделяет тред от `chat_id`, поэтому плагин сам разбирает хвост `:<digits>`). Из отдельного процесса вложения недоставимы (у Bot API нет upload-эндпоинта в этом плагине) — вместо файла в текст добавляется пометка о сгенерированных вложениях. Внутри гейтвея доставка идёт обычным путём через живой адаптер.
...
- **Авторизация (`auth_enabled`):** при включении бот отвечает только **администратору** (совпадение по `auth_admin_login` или запомненному `user_id`), авторизованным пользователям и спискам `allow_from`/`allow_all`. Новому пользователю в **личном** чате бот один раз присылает pairing-код (8 символов, генерируется ядром Hermes) и дублирует запрос с кодом администратору в личку; пока админ не одобрит, неавторизованный пользователь больше **не получает ответов** (повторные сообщения игнорируются), а в групповых чатах/каналах такие сообщения игнорируются всегда. Админ одобряет командой `/auth <код>` → пользователь попадает в список одобренных ядра и сразу получает «Вы авторизованы». Одобренные пользователи хранятся ядром в `~/.hermes/pairing/ym-approved.json` (переживают перезапуск), отозвать доступ можно через CLI `hermes pairing list/revoke`. Админ авто-одобряет себя в этот список при первом обращении. Кода и неверные `/auth` запросы не трогают счётчик неудачных попыток ядра — плагин сверяет код только с выданными им (в памяти, TTL 1 ч как у ядра; после рестарта шлюза активные коды протухают — пользователь запрашивает новый). Требует ссылку на ядро (`gateway_runner`, проставляется автоматически для этого плагина).
